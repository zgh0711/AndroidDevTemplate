...
/**
 * 带侧滑菜单功能的布局
 * http://cherylgood.cn/articles/2017/06/07/1496811955547.html
 */
...
public class SwipeMenuLayout extends ViewGroup {
        ...
        try {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (attr == R.styleable.SwipeMenuLayout_leftMenuView) {
                    mLeftViewResID =
                            typedArray.getResourceId(R.styleable.SwipeMenuLayout_leftMenuView,
                                                     -1);
                } else if (attr == R.styleable.SwipeMenuLayout_rightMenuView) {
                    mRightViewResID =
                            typedArray.getResourceId(R.styleable.SwipeMenuLayout_rightMenuView,
                                                     -1);
                } else if (attr == R.styleable.SwipeMenuLayout_contentView) {
                    mContentViewResID =
                            typedArray.getResourceId(R.styleable.SwipeMenuLayout_contentView,
                                                     -1);
                } else if (attr == R.styleable.SwipeMenuLayout_canLeftSwipe) {
                    mCanLeftSwipe =
                            typedArray.getBoolean(R.styleable.SwipeMenuLayout_canLeftSwipe,
                                                  true);
                } else if (attr == R.styleable.SwipeMenuLayout_canRightSwipe) {
                    mCanRightSwipe =
                            typedArray.getBoolean(R.styleable.SwipeMenuLayout_canRightSwipe,
                                                  true);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (count > 1) {
            for (int i = 0; i < count; i++) {
                ...
                if (lp.width == LayoutParams.MATCH_PARENT) {
                    final int width =
                            Math.max(0, getMeasuredWidth() - lp.leftMargin - lp.rightMargin);
                    childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(width, MeasureSpec.EXACTLY);
                } else {
                    childWidthMeasureSpec =
                            getChildMeasureSpec(widthMeasureSpec, lp.leftMargin + lp.rightMargin,
                                                lp.width);
                }
                ...
                if (lp.height == FrameLayout.LayoutParams.MATCH_PARENT) {
                    final int height =
                            Math.max(0, getMeasuredHeight() - lp.topMargin - lp.bottomMargin);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mLeftView != null) {
            ...
            int lLeft = 0 - mLeftView.getMeasuredWidth() + leftViewLp.leftMargin +
                        leftViewLp.rightMargin;
        ...
        if (mRightView != null) {
            ...
            int lLeft =
                    mContentView.getRight() + mContentViewLp.rightMargin + rightViewLp.leftMargin;
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (Math.abs(distanceX) > mScaledTouchSlop * 2 ||
                    Math.abs(getScrollX()) > mScaledTouchSlop * 2) {
